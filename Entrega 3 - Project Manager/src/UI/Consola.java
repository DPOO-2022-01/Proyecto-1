			...
			do {
				...
				if(opcion == 1) {
					...
					if (mediador.getProyectos().size() == 0) {
						...
						for (Proyecto proyecto: mediador.getProyectos()) {
							if (proyecto.getNombre().equals(nombreProyecto)) {
								//proyecto es el Proyecto al que se le est� registrando una actividad
								...
								System.out.println("Selecciona el participante que realiza� la actividad: \n");
								int j = 1;
								...
								//TODO Implementar la adici�n del participante al hashMap de tiempoParticipante
								...
								if (tipo.getTiempoParticipantes().containsKey(nombreParticipante) == true) {
									...
									//Se sum� el tiempo pasado con el actual y se a�adi� al array del participante
									tipo.getTiempoParticipantes().get(nombreParticipante).set(0, duracionMinutos+tiempoAnterior);
									//Se sum� la participaci�n anterior + 1 y se a�adi� al array del participante
